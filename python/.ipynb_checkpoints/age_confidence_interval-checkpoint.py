...
#age tracking layers
model.add(Convolution2D(90, (1, 1), name='predictions')) # change to 101 for pretrained weights
model.add(Flatten())
...
img_list = []
for i in range(0, 1000):#len(filenames)):
...
model.fit(img_train, age_train, batch_size=32, validation_split=0.2, epochs=1, verbose=1)
